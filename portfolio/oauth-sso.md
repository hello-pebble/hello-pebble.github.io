...
- **검증 방식** Compose로 띄운 모듈에 직접 요청해 확인했고, 매칭 도메인의 동시 경쟁은 10스레드 테스트로 매칭이 정확히 1건만 생성됨을 확인했습니다.
...
## 04. 락을 잡았더니, 이번엔 데드락이 보였습니다 {#concurrency}

인증 경계를 확인한 뒤 매칭 도메인으로 넘어가자 다른 종류의 동시성 문제가 나왔습니다. 두 사용자가 서로를 동시에 선택하면 매칭이 누락되거나(Lost Match) 중복 생성될(Double Matching) 수 있습니다. 사용자 쌍마다 락을 잡는 것이 자연스러운 해법인데, **그 해법이 곧바로 데드락을 만듭니다.** 두 요청이 서로 반대 순서로 락을 잡으면 둘 다 멈춥니다.

해결은 락 자체가 아니라 락의 **순서**에 있었습니다. 항상 작은 ID부터 획득하면 순환 대기가 성립할 수 없고, 데드락이 구조적으로 불가능해집니다. 이 Ordered Locking 원칙은 구현 전에 설계 문서로 먼저 세웠습니다.

| 구분 | 내용 |
| :--- | :--- |
| **검증** | 10개 스레드가 두 사용자를 동시에 상호 선택하는 테스트에서 매칭이 정확히 1건만 생성되는 것을 확인했습니다. |
| **마무리** | 락 해제를 try-finally로 보장하고 락 범위를 상태 변경 구간으로만 최소화했습니다. |
