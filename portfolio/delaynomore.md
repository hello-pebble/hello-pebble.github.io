...
"계획을 세우는 것보다 계획을 계속 고치는 과정이 실행을 늦춘다"는 가설에서 출발해, AI와 대화해 하루 단위 계획을 만들고 **확정 후에는 완료 체크와 다음 날 이월만 허용하는** 실행 중심 서비스를 기획부터 데모 배포까지 혼자 만들었습니다.

v0.1.0부터 v0.17.0까지 완성된 기능 목록을 향해 달리는 대신, **버전마다 직전 버전이 드러낸 문제 하나를 정해 풀었습니다.** 프론트 화면에만 있어 우회 가능하던 잠금 규칙을 서버 강제(409)로 옮기고, 20~30초의 AI 응답 대기를 SSE 스트리밍과 patch 교환으로 바꾸고, 에이전트가 좋아졌는지 육안으로 판단하던 것을 676회 실측 평가 하네스로 교체한 것이 대표적입니다.

릴리스마다 CHANGELOG에 무엇을 왜 바꿨는지 남기고 QA 체크리스트를 실제로 수행한 결과를 기록했으며, 잠금 규칙의 서버 강제·스트리밍 폴백·PostgreSQL 재시작 복원은 자동 테스트 317건으로 고정했습니다.
