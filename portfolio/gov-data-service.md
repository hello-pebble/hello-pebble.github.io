...
폐쇄망이라 외부 SaaS 호출은 선택지에 없었고, 기존 Java 문서 처리 기능만으로는 필요한 파싱 범위를 구현하기 어려웠습니다. 그래서 기존 Java(Spring MVC) 서버는 유지하면서 Python의 로컬 문서 처리 기능을 쓸 수 있도록, 두 실행 환경을 네트워크가 아닌 **같은 서버 안의 서브프로세스**로 연결했습니다.
