...
폐쇄망이라 외부 SaaS 호출은 선택지에 없었습니다. HWP 파싱은 기존에 쓰던 Python 코드가 담당하고 있었는데, 그 버전은 폐쇄망에서는 사용할 수 없어 그대로 들여올 수 없었습니다. 그래서 폐쇄망에서 동작하는 구성으로 Python 스크립트와 가상환경을 Java 서버 로컬에 다시 갖추고, 기존 Java(Spring MVC) 서버는 유지한 채 두 실행 환경을 네트워크가 아닌 **같은 서버 안의 서브프로세스**로 연결했습니다.
