---
layout: default
title: "DelayNoMore — 시스템 아키텍처 & 배포"
permalink: /portfolio/1/architecture/
category: portfolio
tags: [SpringBoot, Docker, CI/CD, OracleCloud, Portfolio]
---

# 시스템 아키텍처 &amp; 배포 <span class="badge outline">DelayNoMore</span>

<span class="section-note">← [DelayNoMore 개요로 돌아가기]({{ '/portfolio/1/' | relative_url }})</span>

> 프론트엔드·백엔드·AI 프록시를 **하나의 컨테이너**로 묶고, **빌드는 한 번만** 수행해 어디서든 이미지를 받아 실행하는 구조를 택했습니다. 저사양 무료 VM에서도 흔들리지 않고 돌아가게 만드는 것이 목표였습니다.
...
- **레이어 분리**: `domain/{ai,plan}` + `global/{response,error,config}` 구조로, 컨트롤러는 검증·호출만 담당하고 비즈니스 로직은 서비스 계층으로 분리했습니다. 예외는 `BusinessException(ErrorCode)` + `GlobalExceptionHandler` 한 곳으로 통일했습니다.
- **표준화된 계약**: URL 버저닝(`/api/v1`)과 공통 응답 래퍼 `{ success, data, error }`를 도입해 검증 실패는 `error.fieldErrors`, 분기는 `error.code`로 판별합니다.
...
- **키 관리**: 배포 스크립트가 `~/.delaynomore.env`를 자동 로드해, API 키를 최초 1회만 파일(chmod 600)로 저장하면 이후 배포에서 셸 히스토리에 키가 남지 않습니다.
...
<span class="section-note">관련: [규칙의 소유권을 서버로]({{ '/portfolio/1/backend-ownership/' | relative_url }}) · [데이터 영속화 & 동시성]({{ '/portfolio/1/persistence/' | relative_url }}) · [AI 통합]({{ '/portfolio/1/ai-engineering/' | relative_url }})</span>
