...
- **생산성 향상**: 인증을 게이트웨이와 중앙 인증 서버로 모아, **개별 마이크로서비스가 인증 로직을 중복 개발할 필요가 없는 구조**를 만들었습니다.
- **성능 구조 개선**: 게이트웨이의 무상태 검증 아키텍처로 **개별 API 서버가 인증을 위해 DB를 조회하거나 세션을 관리할 필요를 제거**했습니다.
